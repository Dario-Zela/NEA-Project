...
namespace Models.WorldGen
{
        ...
        public float[,] GenerateNoiseMap(int mapDepth, int mapWidth, float Scale, int seed, int Octaves, float Persistance, float Lacunarity)
        {
            ...
            for (int zIndex = 0; zIndex < mapDepth; zIndex++)
            {
                for (int xIndex = 0; xIndex < mapWidth; xIndex++)
                {
                    ...
                    float noise = perlin.OctaveNoiseGen(SampleIndexX, SampleIndexZ);
                    if (noise > maxNoise)
                    {
                        maxNoise = noise;
                    }
    ...
    */
    ...
    public class WorldCreator
    {
        ...
        public float[,] GenMap(float Scale, int seed)
        {
            MapGen gen = new MapGen();
            return gen.GenerateNoiseMap(Constants.WORLD_HEIGHT, Constants.WORLD_HEIGHT, Scale, seed, 8, (float)0.5, 2);
        }
